...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  if (document.getElementById('summer-collection') && document.getElementById('winter-collection')) {
    ...
    function renderProductCard(product, idx) {
      ...
      const sizes = (product.sizes || []).map(size => `<span class='badge bg-secondary mx-1'>${size}</span>`).join(' ');
      ...
      return `
        <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-4 animate__animated animate__fadeInUp" style="animation-delay:${idx * 0.1}s;">
          <div class="card h-100 shadow-sm">
            <img src="${img}" class="card-img-top" alt="${name}" style="height:220px;object-fit:cover;">
            <div class="card-body d-flex flex-column">
              <h5 class="card-title">${name}</h5>
              <div class="mb-1"><strong>${lang === 'ar' ? 'المقاسات' : 'Sizes'}:</strong> ${sizes}</div>
              <div class="mb-2"><strong>${lang === 'ar' ? 'الألوان' : 'Colors'}:</strong> ${colors}</div>
              <a href="product.html?id=${product.id}" class="btn btn-outline-primary mt-auto w-100">${lang === 'ar' ? 'عرض التفاصيل' : 'View Details'}</a>
            </div>
          </div>
        </div>
      `;
